    ...
    if (exportMode === "持ち出し用") {
      ...
      const raceDayRows = cardRows
        .map((row) => {
          ...
          return `<tr><td>${escapeHtml(cardDistanceLabel(row))}</td><td>${escapeHtml(row.etaMinutes == null ? "-" : addMinutesToClock("00:00", row.etaMinutes))}</td><td>${escapeHtml(formatPace(row.adjustedLapSec).replace("/km", ""))}</td><td>${escapeHtml(shortCheckMemo(row).replace(/ \/ /g, "・"))}</td></tr>`;
        })
        .join("");
      const columns = getPaceComparisonColumns();
      ...
      const comparisonRows = comparisonPoints.map((point) => {
        const cells = columns.map((column) => {
          const row = getComparisonRow(column.rows, point.km);
          return `<td><b>${escapeHtml(row ? formatPace(row.adjustedLapSec).replace("/km", "") : "-")}</b><br><span>${escapeHtml(row?.etaMinutes == null ? "-" : addMinutesToClock("00:00", row.etaMinutes))}</span></td>`;
        }).join("");
        return `<tr><td>${escapeHtml(point.label)}</td>${cells}</tr>`;
      }).join("");
      const raceDayPanel = `<section class="panel raceDay"><table><thead><tr><th>距離</th><th>通過</th><th>ペース</th><th>確認</th></tr></thead><tbody>${raceDayRows}</tbody></table></section>`;
      const comparisonPanel = `<section class="panel threePlan"><table class="compare"><thead><tr><th class="corner"><span>Time</span><b>km</b></th>${headerCells}</tr></thead><tbody>${comparisonRows}</tbody></table></section>`;
      const ticket = (label: string) => `<div class="ticket"><div class="foldLine"></div><div class="ticketLabel">${label}</div><header class="ticketHead"><div class="raceInfo"><h2>CHEBIS RUN</h2><b>${escapeHtml(selectedRace?.name ?? "")}</b><span>ロス${escapeHtml(selectedRace?.lostTimeMin ?? "0")}分 / 開始${escapeHtml(getRealStartTime(selectedRace))}</span></div><div class="titleBar"><strong>3PLAN</strong><strong>RACE DAY</strong></div></header><main class="ticketBody">${comparisonPanel}${raceDayPanel}</main><p class="foot">公式情報は大会前に確認してください。完走保証ではありません。</p></div>`;
      const html = `<!doctype html><html><head><meta charset="utf-8"><style>@page{size:A4 portrait;margin:10mm 7mm}*{box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'Helvetica Neue',sans-serif;color:#050606;margin:0}h1{font-size:8px;margin:0 0 3mm;color:#111}.sheet{width:100%;display:flex;flex-direction:column;align-items:center;gap:14mm}.ticket{position:relative;width:182mm;height:55mm;border:1.2px solid #111;padding:2.2mm;overflow:hidden;break-inside:avoid;page-break-inside:avoid}.ticketLabel{position:absolute;right:2mm;top:1.5mm;font-size:5px;font-weight:900}.foldLine{position:absolute;top:0;bottom:0;left:50%;border-left:1px dashed #777}.ticket:first-child::after{content:"切り取り";position:absolute;left:0;right:0;bottom:-7mm;border-bottom:1px dashed #777;text-align:center;color:#777;font-size:5px}.ticketHead{height:14mm;display:grid;grid-template-columns:58mm 1fr;gap:3mm;align-items:start}.raceInfo h2{font-size:8.5px;line-height:1;margin:0 0 .8mm;font-weight:900;letter-spacing:.2px}.raceInfo b{display:block;font-size:6px;line-height:1.15;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.raceInfo span{display:block;font-size:6px;line-height:1.15;font-weight:900}.titleBar{height:12mm;background:#050505;color:#fff;display:grid;grid-template-columns:1fr 1fr;align-items:center;text-align:center;font-size:18px;line-height:1;font-weight:900;letter-spacing:.5px}.ticketBody{display:grid;grid-template-columns:91mm 91mm;margin-top:1mm}.panel{min-width:0;padding:0 1.2mm}.threePlan{padding-left:0}.raceDay{padding-right:0}table{width:100%;height:34.5mm;border-collapse:collapse;table-layout:fixed;font-size:7px}th,td{border:1px solid #111;padding:.25mm .45mm;text-align:center;vertical-align:middle;line-height:1.02;font-weight:900;overflow:hidden}th{font-size:7px;background:#fff}.compare th{height:7mm}.compare td:first-child,.raceDay td:first-child{font-size:9px}.compare td{font-size:8px}.compare span{font-size:6.2px}.compare .corner{position:relative;width:18mm;text-align:left;vertical-align:bottom}.compare .corner::before{content:"";position:absolute;left:0;top:0;width:140%;border-top:1px solid #111;transform:rotate(42deg);transform-origin:left top}.compare .corner span{position:absolute;right:1.2mm;top:1.4mm;font-size:8px}.compare .corner b{position:absolute;left:2mm;bottom:1.2mm;font-size:8px}.raceDay th:first-child,.raceDay td:first-child{width:18mm}.raceDay th:nth-child(2),.raceDay td:nth-child(2){width:21mm}.raceDay th:nth-child(3),.raceDay td:nth-child(3){width:21mm}.raceDay th:nth-child(4),.raceDay td:nth-child(4){width:27mm;font-size:8px}.foot{position:absolute;left:2.2mm;bottom:1mm;font-size:4.2px;line-height:1;color:#333;margin:0}@media print{body{margin:0}tr{break-inside:avoid}.ticket{break-inside:avoid;page-break-inside:avoid}}</style></head><body><h1>RUN Finish Planner / レース本番用 名刺二つ折り v20260705-2</h1><div class="sheet">${ticket("本番用")}${ticket("控え")}</div></body></html>`;
      if (Platform.OS === "web") {
        const web = globalThis as any;
