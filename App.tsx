...
export default function App() {
  ...
  async function exportCsv() {
    ...
    if (exportMode === "持ち出し用") {
      ...
      if (Platform.OS === "web") {
        ...
        link.href = url;
        link.download = `${safeName}-race-day-comparison.csv`;
        link.click();
        web.URL.revokeObjectURL(url);
        return;
      }
      const uri = `${FileSystem.documentDirectory}${safeName}-race-day-comparison.csv`;
  ...
  async function exportPdf() {
    ...
    if (exportMode === "持ち出し用") {
      ...
      const cardDistanceLabel = (row: PaceRow) => {
        if (Math.abs(row.km - raceDistance) < 0.01) return "ゴール";
        return `${distanceLabel(row.gate?.distanceKm ?? row.km)}km`;
      };
      const raceDayRows = cardRows
        .map((row) => {
          ...
          return `<tr><td>${escapeHtml(cardDistanceLabel(row))}</td><td><strong>${escapeHtml(row.etaMinutes == null ? "-" : addMinutesToClock("00:00", row.etaMinutes))}</strong><small>${escapeHtml(formatDuration(intervalSec).replace(/^0:/, ""))}</small></td><td>${escapeHtml(shortCheckMemo(row).replace(/ \/ /g, "・"))}</td></tr>`;
        })
        .join("");
      const columns = getPaceComparisonColumns();
      const comparisonPoints = comparisonPointLabels()
        ...
        .slice(0, maxCardRows);
      const headerCells = columns.map((column) => `<th>${escapeHtml(column.label)}<br><strong>${escapeHtml(formatDuration(column.targetSec).replace(/^0/, ""))}</strong></th>`).join("");
      const comparisonRows = comparisonPoints.map((point) => {
        const cells = columns.map((column) => {
          const row = getComparisonRow(column.rows, point.km);
          return `<td><b>${escapeHtml(row?.etaMinutes == null ? "-" : addMinutesToClock("00:00", row.etaMinutes))}</b><br><span>${escapeHtml(row ? formatPace(row.adjustedLapSec).replace("/km", "") : "-")}</span></td>`;
        }).join("");
        return `<tr><td>${escapeHtml(point.label)}</td>${cells}</tr>`;
      }).join("");
      const raceSummary = `<div class="summary"><b>${escapeHtml((selectedRace?.name ?? "").replace(/2026$/, ""))}</b><span>目標 ${escapeHtml(goalTimeLabel)} / 平均 ${escapeHtml(formatPace(basePace).replace("/km", ""))}</span><span>開始 ${escapeHtml(getRealStartTime(selectedRace))} / 余裕 最小${escapeHtml(formatMinutesLabel(minMargin))}</span></div>`;
      const raceDayPanel = `<section class="face raceDay"><h2>CHEBIS RUN</h2><p class="sub">RACE DAY</p>${raceSummary}<table><thead><tr><th>km</th><th>通過</th><th>確認</th></tr></thead><tbody>${raceDayRows}</tbody></table><p class="foot">公式情報は大会前に確認。完走保証ではありません。</p></section>`;
      const comparisonPanel = `<section class="face threePlan"><h2>CHEBIS RUN</h2><p class="sub">3 PLAN</p><div class="summary compact"><b>安全・目標・攻める</b><span>通過予定を比較</span></div><table class="compare"><thead><tr><th>km</th>${headerCells}</tr></thead><tbody>${comparisonRows}</tbody></table></section>`;
      const ticket = (label: string) => `<div class="ticket ${label === "控え" ? "spare" : ""}"><div class="foldLine"><span>山折り</span></div><span class="ticketLabel">${label}</span>${raceDayPanel}${comparisonPanel}</div>`;
      const html = `<!doctype html><html><head><meta charset="utf-8"><style>@page{size:A4 portrait;margin:8mm}*{box-sizing:border-box}html,body{margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,'Helvetica Neue',sans-serif;color:#111;-webkit-print-color-adjust:exact;print-color-adjust:exact}h1{font-size:7.5px;margin:0 0 4mm;color:#111;font-weight:900}.sheet{width:100%;display:flex;flex-direction:column;align-items:center;gap:12mm}.ticket{position:relative;width:110mm;height:91mm;display:grid;grid-template-columns:55mm 55mm;border:1.5px solid #163763;overflow:hidden;break-inside:avoid;page-break-inside:avoid;background:#fff}.ticketLabel{position:absolute;right:2mm;top:1.25mm;font-size:6px;line-height:1;font-weight:1000;color:#163763;z-index:8;background:#fff;padding:0 .6mm}.foldLine{position:absolute;top:0;bottom:0;left:55mm;border-left:1px dashed #6c86aa;z-index:5}.foldLine span{position:absolute;top:1.1mm;left:-4.8mm;font-size:4.2px;line-height:1;color:#6c86aa;background:#fff;padding:0 .6mm}.face{position:relative;width:55mm;height:91mm;padding:2.2mm 1.8mm 2.7mm;overflow:hidden}.face h2{font-size:10.8px;line-height:.95;margin:0;color:#173863;font-weight:1000;letter-spacing:.15px}.sub{font-size:5.7px;line-height:1;margin:.8mm 0 1.45mm;color:#173863;font-weight:1000}.summary{background:#f0eee9;padding:1.25mm 1.35mm;margin:0 0 1.45mm;min-height:9.2mm}.summary.compact{min-height:8.2mm}.summary b{display:block;font-size:6.4px;line-height:1.12;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:1000}.summary span{display:block;font-size:5.25px;line-height:1.18;font-weight:900}table{width:100%;border-collapse:collapse;table-layout:fixed}th,td{border:1.15px solid #173863;text-align:left;vertical-align:middle;line-height:1.02;font-weight:950;overflow:hidden}th{background:#edf4f1;color:#173863;font-size:5.8px;padding:.58mm .48mm}.raceDay table{height:66.2mm}.threePlan table{height:66mm}.raceDay th:nth-child(1),.raceDay td:nth-child(1){width:9.8mm}.raceDay th:nth-child(2),.raceDay td:nth-child(2){width:16.8mm}.raceDay th:nth-child(3),.raceDay td:nth-child(3){width:auto}.raceDay td{padding:.42mm .48mm}.raceDay td:nth-child(1){font-size:7.9px;font-weight:1000}.raceDay td:nth-child(3){font-size:6.3px;font-weight:1000}.raceDay td strong{display:block;font-size:10.4px;line-height:.94;font-weight:1000;letter-spacing:.1px}.raceDay td small{display:block;font-size:4.95px;color:#31403b;line-height:1;margin-top:.28mm;font-weight:1000}.compare th:first-child,.compare td:first-child{width:8.7mm}.compare th{font-size:5.15px;text-align:center;padding:.5mm .18mm}.compare th:first-child{text-align:left}.compare th strong{font-size:5.05px}.compare td{font-size:5.8px;text-align:center;padding:.36mm .16mm}.compare td:first-child{text-align:left;font-size:6.5px;font-weight:1000}.compare td b{display:block;font-size:7.9px;line-height:.96;font-weight:1000;letter-spacing:.03px}.compare td span{display:block;font-size:4.85px;color:#31403b;line-height:1;margin-top:.25mm;font-weight:900}.foot{position:absolute;left:1.8mm;right:1.8mm;bottom:1.2mm;margin:0;font-size:3.9px;line-height:1.08;color:#555}.spare{margin-top:0}@media print{body{margin:0}tr{break-inside:avoid}.ticket{break-inside:avoid;page-break-inside:avoid}}</style></head><body><h1>RUN Finish Planner / レース本番用 縦型名刺二つ折り v20260705-readable2</h1><div class="sheet">${ticket("本番用")}${ticket("控え")}</div></body></html>`;
      if (Platform.OS === "web") {
        const web = globalThis as any;
  ...
  function renderPlanTab() {
    ...
    return (
      <>
        <Segment value={activePlanSection} values={["作成", "ペース表", "印刷・出力"]} onChange={setPlanSection} />
        ...
        {activePlanSection === "印刷・出力" && (
          <Card>
            ...
            <Text style={styles.helpText}>
              {exportMode === "持ち出し用"
                ? "A4縦1枚に、本番用と控えを上下に印刷します。各カードは91mm×55mmの名刺サイズ2面で、中央の点線を山折りにできます。PDF見出しが「名刺二つ折り」になっていれば最新版です。"
  ...
  function renderPaceTable() {
    ...
    const paceSectionLabel = (startKm: number, endKm: number) => {
      return `${distanceLabel(startKm)}-${distanceLabel(endKm)}km`;
    };
