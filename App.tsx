...
export default function App() {
  ...
  async function exportPdf() {
    ...
    if (exportMode === "持ち出し用") {
      ...
      const raceDayPanel = `<section class="face raceDay"><h2>CHEBIS RUN</h2><p class="sub">RACE DAY</p><i>本番用</i>${raceSummary}<table><thead><tr><th>km</th><th>通過</th><th>確認</th></tr></thead><tbody>${raceDayRows}</tbody></table><p class="foot">公式情報は大会前に確認。完走保証ではありません。</p></section>`;
      const comparisonPanel = `<section class="face threePlan"><h2>CHEBIS RUN</h2><p class="sub">3 PLAN</p><i>本番用</i><div class="summary"><b>安全・目標・攻める</b><span>通過予定だけ確認</span></div><table class="compare"><thead><tr><th>km</th>${headerCells}</tr></thead><tbody>${comparisonRows}</tbody></table></section>`;
      const ticket = (label: string) => `<div class="ticket ${label === "控え" ? "spare" : ""}"><div class="foldLine"><span>山折り</span></div><div class="cutLine"><span>切り取り</span></div><span class="ticketLabel">${label}</span>${raceDayPanel.replace("本番用", label)}${comparisonPanel.replace("本番用", label)}</div>`;
      const html = `<!doctype html><html><head><meta charset="utf-8"><style>@page{size:A4 portrait;margin:12mm 10mm}*{box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'Helvetica Neue',sans-serif;color:#111;margin:0}h1{font-size:9px;margin:0 0 5mm;color:#111}.sheet{width:100%;display:flex;flex-direction:column;align-items:center;gap:20mm}.ticket{position:relative;width:110mm;height:91mm;display:grid;grid-template-columns:55mm 55mm;border:1.4px solid #163763;overflow:hidden;break-inside:avoid;page-break-inside:avoid;background:#fff}.ticketLabel{position:absolute;right:1.8mm;top:1.4mm;font-size:5.4px;font-weight:950;color:#163763}.foldLine{position:absolute;top:0;bottom:0;left:55mm;border-left:1px dashed #6c86aa;z-index:5}.foldLine span{position:absolute;top:1mm;left:-5mm;font-size:4px;color:#6c86aa;background:#fff}.cutLine{position:absolute;left:0;right:0;bottom:-6mm;border-bottom:1px dashed #6c86aa;text-align:center;color:#6c86aa;font-size:4px}.cutLine span{position:relative;top:2.4mm;background:#fff;padding:0 2mm}.face{position:relative;width:55mm;height:91mm;padding:2.4mm 2mm 2.8mm;overflow:hidden}.face h2{font-size:9.7px;line-height:1;margin:0;color:#173863;font-weight:950;letter-spacing:.25px}.sub{font-size:5.5px;line-height:1;margin:.7mm 0 1.5mm;color:#173863;font-weight:950}.face i{position:absolute;right:2mm;top:2.4mm;font-style:normal;font-size:4.5px;font-weight:950;color:#173863}.summary{background:#f0eee9;padding:1.35mm 1.5mm;margin:0 0 1.6mm;min-height:8.4mm}.summary b{display:block;font-size:5.9px;line-height:1.12;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.summary span{display:block;font-size:5.1px;line-height:1.18;font-weight:850}table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:6px}th,td{border:1.1px solid #173863;padding:.65mm .5mm;text-align:left;vertical-align:middle;line-height:1.04;font-weight:900;overflow:hidden}th{background:#edf4f1;color:#173863;font-size:5.4px}.raceDay table{height:66mm}.threePlan table{height:65mm}.raceDay th:nth-child(1),.raceDay td:nth-child(1){width:9.2mm}.raceDay th:nth-child(2),.raceDay td:nth-child(2){width:15.8mm}.raceDay th:nth-child(3),.raceDay td:nth-child(3){width:auto}.raceDay td:nth-child(1){font-size:7.2px}.raceDay td strong{display:block;font-size:8.7px;line-height:1;font-weight:1000;letter-spacing:.1px}.raceDay td small{display:block;font-size:4.7px;color:#4b5c57;line-height:1.02;margin-top:.35mm}.compare th:first-child,.compare td:first-child{width:8.4mm}.compare th{font-size:5px;text-align:center;padding:.55mm .25mm}.compare th strong{font-size:4.8px}.compare td{font-size:5.6px;text-align:center;padding:.48mm .25mm}.compare td:first-child{text-align:left;font-size:6.2px}.compare td b{display:block;font-size:7.4px;line-height:1;font-weight:1000;letter-spacing:.05px}.compare td span{display:block;font-size:4.3px;color:#4b5c57;line-height:1;margin-top:.35mm}.foot{position:absolute;left:2mm;right:2mm;bottom:1.35mm;margin:0;font-size:3.8px;line-height:1.1;color:#555}.spare{margin-top:0}@media print{body{margin:0}tr{break-inside:avoid}.ticket{break-inside:avoid;page-break-inside:avoid}}</style></head><body><h1>RUN Finish Planner / レース本番用 縦型名刺二つ折り v20260705-readable</h1><div class="sheet">${ticket("本番用")}${ticket("控え")}</div></body></html>`;
      if (Platform.OS === "web") {
        const web = globalThis as any;
