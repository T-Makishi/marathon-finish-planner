...
export function buildPrintLayout(plan: Plan): PrintLayout {
  ...
  } else pages.push(...chunks(cards, 2).map(pair => {
    const pairHeight = Math.max(...pair.map(card => card.height));
    return { kind: 'cards' as const, cards: pair.map(card => ({ ...card, height: pairHeight })), folded: pair.length === 2 };
  }));
  ...
  if (!isWrist && height > 135) warnings.push(`文字の大きさを保つため、カードの高さを${height}mmに延長しました。`);
  if (groups.length > 1) warnings.push('全地点を1面に収められないため、情報を省略せず続きの面に分けました。');
  const details: DetailTable[] = [];
  if (plan.cardGates) {
    const sortedGates = [...plan.gates].sort((a, b) => numberValue(a.km) - numberValue(b.km));
    const emphasized = sortedGates.map(() => true);
    if (plan.limit) emphasized.push(true);
    const rows = sortedGates.map(g => [
      `${numberValue(g.km) === 0 ? "スタート (0km)" : `${g.km}km`}\n${g.name}\n${g.kind === 'advisory' ? '競技中止勧告' : '正式関門'}`,
  ...
  return { pages, width, height, cardCount: isWrist ? cards.length : pages.filter(page => page.kind === 'cards').length, detailCount: pages.filter(p => p.kind === 'detail').length, warnings };
}
