...
export type PrintPage = { kind: 'cards'; cards: CarryCard[] } | { kind: 'detail'; table: DetailTable };
...
export function buildPrintLayout(plan: Plan): PrintLayout {
  ...
  for (const kind of ['single', 'compare'] as const) {
    ...
    groups.forEach((points, i) => cards.push({ kind, width, height, points, part: i + 1, parts: groups.length }));
  }
  ...
  return { pages, width, height, cardCount: cards.length, detailCount: pages.filter(p => p.kind === 'detail').length, warnings };
}
export function buildCardHtml(plan: Plan, createdAt = new Date().toISOString(), options: { previewPage?: number; preview?: boolean } = {}): string {
  ...
  const renderedPages = layout.pages.map((page, index) => {
    ...
    const sub = page.kind === 'cards' ? `${layout.width} × ${layout.height}mm ／ このページは${page.cards.length}枚` : `${plan.raceName} ／ ${plan.name}`;
    return `<article class="sheet" data-page="${index + 1}"><div class="sheet-head"><div><h1>${esc(title)}</h1><p>${esc(sub)}</p></div><b>${index + 1} / ${layout.pages.length}</b></div>${page.kind === 'cards' ? `<div class="cut-guide"><span>点線で切り取り、100%で印刷してください。</span><span class="scale">50mm</span></div><div class="card-grid">${page.cards.map(c => `<div class="card-wrap"><div class="dimension">${c.kind === 'single' ? '本番案' : '比較用'} · ${c.width} × ${c.height}mm</div>${card(c)}</div>`).join('')}</div><div class="page-note">${esc(plan.name)} ／ ${esc(basis)}<br>関門・補給・コース補正は、選択した場合だけ別紙で出力します。<br>大会の最新要項を確認し、切り取ったカードの読みやすさを確かめてください。</div>` : `<p class="detail-subtitle">${esc(page.table.subtitle)}</p><table class="detail"><colgroup>${page.table.widths.map(w => `<col style="width:${w}mm">`).join('')}</colgroup><thead><tr>${page.table.headers.map(h => `<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${page.table.rows.map((row, i) => `<tr style="height:${page.table.heights[i]}mm">${row.map(cell => `<td>${esc(cell).replace(/\n/g, '<br>')}</td>`).join('')}</tr>`).join('')}</tbody></table>`}<div class="sheet-foot"><span>PCSAPO / マキシ企画</span><span>作成 ${esc(createdAt.slice(0, 10))} · ${index + 1}/${layout.pages.length}</span></div></article>`;
  }).join('');
