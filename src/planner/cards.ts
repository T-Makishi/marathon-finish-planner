...
export function exportProblems(plan: Plan): string[] {
  ...
  if (displayPoints(plan).some(p => pointHeight(plan, p, plan.cardFormat === 'wrist' && plan.cardMode === 'single') > 60.5)) errors.push('同じ距離の関門が多すぎます。関門を整理してください。');
...
export function buildPrintLayout(plan: Plan): PrintLayout {
  ...
  for (const point of displayPoints(plan)) {
    ...
    if (group.length && used + height > (isWrist ? 78 : 60.5) + 0.001) { groups.push(group); group = []; used = 0; }
