...
export function exportProblems(plan: Plan): string[] {
  ...
  const wrist = plan.cardFormat === 'wrist' && plan.cardMode === 'single';
  if (displayPoints(plan).some(p => pointHeight(plan, p, wrist) > (wrist ? 78 : 120.5))) errors.push('同じ距離の関門が多すぎます。関門を整理してください。');
...
export function buildPrintLayout(plan: Plan): PrintLayout {
  ...
  const width = isWrist ? 50 : 85;
  const rowLimit = isWrist ? 78 : 120.5;
  ...
  for (const [i, points] of groups.entries()) {
    for (const kind of ['single', 'compare'] as const) {
      ...
      cards.push({ kind, width, height: cardHeight(points), points, part: i + 1, parts: groups.length });
    }
  }
...
export function buildCardHtml(plan: Plan, createdAt = new Date().toISOString(), options: { previewPage?: number; preview?: boolean } = {}): string {
  ...
  const renderedPages = layout.pages.map((page, index) => {
    ...
    const sub = page.kind === 'cards' ? page.folded ? `二つ折り1枚 ／ 広げて170 × ${pageHeight}mm → 折って85 × ${pageHeight}mm` : `${layout.width} × ${pageHeight}mm ／ このページは${page.cards.length}枚` : `${plan.raceName} ／ ${plan.name}`;
