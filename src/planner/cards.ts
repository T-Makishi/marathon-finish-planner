...
function detailTables(title: string, subtitle: string, headers: string[], widths: number[], rawRows: string[][]): DetailTable[] {
  ...
  return [{ title, subtitle, headers, widths, rows, heights, continuation: 1 }];
}
function packDetails(tables: DetailTable[]): PrintPage[] {
  ...
  for (const table of tables) {
    ...
    while (start < table.rows.length) {
      ...
      current.push({ ...table, rows: table.rows.slice(start, end), heights: table.heights.slice(start, end), continuation: continuation++ });
...
export function buildPrintLayout(plan: Plan): PrintLayout {
  ...
  if (plan.cardGates) {
    ...
    details.push(...detailTables('関門・制限時間の確認表', '締切・通過予定は時刻。余裕は登録情報に基づく計算です。', ['地点・区分', '締切時刻', '通過予定時刻', '残り余裕'], [51, 31, 52, 52], rows));
...
export function buildCardHtml(plan: Plan, createdAt = new Date().toISOString(), options: { previewPage?: number; preview?: boolean } = {}): string {
  ...
  const renderedPages = layout.pages.map((page, index) => {
    ...
    const title = page.kind === 'cards' ? '携帯用ペースカード' : '大会当日の確認資料';
