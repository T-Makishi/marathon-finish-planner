...
// Carry cards contain only the selected race's standard pace points. Supplemental
// checkpoint/stop/terrain tables have their own explicit, independently paginated pages.
export function displayPoints(plan: Plan): number[] { return cardPoints(numberValue(plan.distance)); }
...
export function exportProblems(plan: Plan): string[] {
  ...
  if (plan.cardGates && plan.gates.some(g => g.name.length > 40)) errors.push('印刷する関門名は40文字以内にしてください。');
...
export type DetailTable = { title: string; subtitle: string; headers: string[]; widths: number[]; rows: string[][]; heights: number[]; continuation: number };
export type PrintPage = { kind: 'cards'; cards: CarryCard[]; folded?: boolean } | { kind: 'detail'; table: DetailTable };
...
function detailPages(title: string, subtitle: string, headers: string[], widths: number[], rawRows: string[][]): PrintPage[] {
  if (!rawRows.length) return [];
  const pages: PrintPage[] = []; let rows: string[][] = [], heights: number[] = [], used = 0;
  const flush = () => { if (rows.length) pages.push({ kind: 'detail', table: { title, subtitle, headers, widths, rows, heights, continuation: pages.length + 1 } }); rows = []; heights = []; used = 0; };
  for (const raw of rawRows) {
    const wrapped = raw.map((text, i) => wrap(text, widths[i]));
    const height = Math.max(...wrapped.map(v => v.length)) * 4.2 + 3;
    if (used + height > 211) flush();
    rows.push(wrapped.map(v => v.join('\n'))); heights.push(height); used += height;
  }
  flush(); return pages;
}
export function buildPrintLayout(plan: Plan): PrintLayout {
  const isWrist = plan.cardMode === 'single' && plan.cardFormat === 'wrist';
  const width = isWrist ? 50 : 85, height = isWrist ? 180 : 135;
  const groups = chunks(displayPoints(plan), isWrist ? 16 : 12);
  ...
  for (const [i, points] of groups.entries()) {
    for (const kind of ['single', 'compare'] as const) {
      ...
      cards.push({ kind, width, height, points, part: i + 1, parts: groups.length });
    }
  }
  const pages: PrintPage[] = chunks(cards, isWrist ? 3 : 2).map(cards => ({ kind: 'cards', cards, folded: plan.cardMode === 'both' }));
  ...
  if (plan.cardGates) {
    const rows = [...plan.gates].sort((a, b) => numberValue(a.km) - numberValue(b.km)).map(g => [
    ...
    pages.push(...detailPages('関門・制限時間の確認表', '締切・通過予定は時刻。余裕は登録情報に基づく計算です。', ['地点・区分', '締切時刻', '通過予定時刻', '残り余裕'], [51, 31, 52, 52], rows));
    if (!rows.length) warnings.push('関門・制限時間が未登録のため、関門一覧は出力しません。');
  }
  if (plan.cardNotes) {
    pages.push(...detailPages('補給・停止の計画', '停止時間はペースカードの累計に反映されています。', ['地点', '停止時間', '関門との順序', 'メモ'], [23, 26, 48, 89], [...plan.stops].sort((a, b) => numberValue(a.km) - numberValue(b.km)).map(s => [`${s.km}km`, `${s.seconds}秒`, s.afterGate ? '同地点の関門通過後' : '同地点の関門通過前', s.memo])));
  }
  if (plan.cardTerrain) {
    pages.push(...detailPages('コース補正の設定', plan.elevation ? 'この補正を計画に適用しています。' : '補正はOFFです。下記は保存済み設定で、計画には適用していません。', ['区間', '補正', 'メモ'], [38, 33, 115], plan.terrain.map(t => [`${t.start}〜${t.end}km`, `${numberValue(t.adjustment) > 0 ? '+' : ''}${t.adjustment}秒/km`, t.memo])));
  }
  return { pages, width, height, cardCount: plan.cardMode === 'both' ? cards.length / 2 : cards.length, detailCount: pages.filter(p => p.kind === 'detail').length, warnings };
}
export function buildCardHtml(plan: Plan, createdAt = new Date().toISOString(), options: { previewPage?: number; preview?: boolean } = {}): string {
  ...
  const renderedPages = layout.pages.map((page, index) => {
    ...
    const title = page.kind === 'cards' ? '携帯用ペースカード' : page.table.title;
    const sub = page.kind === 'cards' ? page.folded ? '二つ折り1枚 ／ 広げて170 × 135mm → 折って85 × 135mm' : `${layout.width} × ${layout.height}mm ／ このページは${page.cards.length}枚` : `${plan.raceName} ／ ${plan.name}`;
    return `<article class="sheet" data-page="${index + 1}"><div class="sheet-head"><div><h1>${esc(title)}</h1><p>${esc(sub)}</p></div><b>${index + 1} / ${layout.pages.length}</b></div>${page.kind === 'cards' ? `<div class="cut-guide"><span>${page.folded ? '外周の破線だけを切り取る。中央は切らずに山折り。' : '外周の破線で切り取り、100%で印刷してください。'}</span><span class="scale">50mm</span></div><div class="card-grid">${page.folded ? `<div class="fold-unit"><div class="fold-labels"><span>表面：本番案</span><span>裏面：3目標比較</span></div><div class="fold-card">${page.cards.map(c => card(c)).join('')}<div class="fold-line"><span>折り線・切らない</span></div></div></div>` : page.cards.map(c => `<div class="card-wrap"><div class="dimension">${c.kind === 'single' ? '本番案' : '比較用'} · ${c.width} × ${c.height}mm</div>${card(c)}</div>`).join('')}</div><div class="page-note">${page.folded ? '<strong>外周のみ切り取り → 印刷面を外側にして中央を山折り（中央は切らない）</strong><br>' : ''}${esc(plan.name)} ／ ${esc(basis)}<br>関門・補給・コース補正は、選択した場合だけ別紙で出力します。<br>大会の最新要項を確認し、切り取ったカードの読みやすさを確かめてください。</div>` : `<p class="detail-subtitle">${esc(page.table.subtitle)}</p><table class="detail${page.table.title === '関門・制限時間の確認表' ? ' gate-table' : ''}"><colgroup>${page.table.widths.map(w => `<col style="width:${w}mm">`).join('')}</colgroup><thead><tr>${page.table.headers.map(h => `<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${page.table.rows.map((row, i) => `<tr style="height:${page.table.heights[i]}mm">${row.map(cell => `<td>${esc(cell).replace(/\n/g, '<br>')}</td>`).join('')}</tr>`).join('')}</tbody></table>`}<div class="sheet-foot"><span>PCSAPO / マキシ企画</span><span>作成 ${esc(createdAt.slice(0, 10))} · ${index + 1}/${layout.pages.length}</span></div></article>`;
  }).join('');
