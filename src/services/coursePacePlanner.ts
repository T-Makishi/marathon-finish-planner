import { RaceDataSection, RaceDataTerrain } from "../data/raceData";
...
function terrainAdjustment(terrain: RaceDataTerrain, climbSecPerKm: number, descentSecPerKm: number) {
  if (terrain === "uphill") return climbSecPerKm;
  if (terrain === "downhill") return descentSecPerKm;
  if (terrain === "rolling") return Math.max(3, Math.round(climbSecPerKm * 0.35));
  if (terrain === "mixed") return Math.max(2, Math.round(climbSecPerKm * 0.2));
  return 0;
}
...
export function calculateFiveKmPacePlan({
  ...
  const seeded = sourceSections.map((section) => {
    ...
    const adjustment = terrainAdjustment(section.terrain, climbSecPerKm, descentSecPerKm);
    return {
      section,
      distance,
      isFirstHalf,
  ...
  return seeded.map((row, index) => {
    ...
    return {
      ...
      confidence: row.section.confidence ?? "low",
